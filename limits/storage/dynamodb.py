...
class DynamoDBStorage(Storage, MovingWindowSupport):
    """
    Rate limit storage with DynamoDB as backend.

    Depends on :pypi:`boto3`.
    """

    STORAGE_SCHEME = ["dynamodb"]
    DEPENDENCIES = ["boto3"]


    def __init__(
        self, uri: str, tablename: str = "limits", **options: Union[int, str, bool]
    ) -> None:
        """
        :param uri: uri of the form ``dynamodb://tablename...``,
         This uri is passed directly to :class:`boto3.dynamodb`
        :param database_name: The database to use for storing the rate limit
         collections.
        :raise ConfigurationError: when the :pypi:`boto3` library is not available
        """

        super().__init__(uri, **options)

        self.lib = self.dependencies["boto3"].module

        self.storage: "boto3.dynamodb" = self.lib.resource('dynamodb')
        self.counters = self.storage.Table("limits_counters")
        self.windows = self.storage.Table("limits_windows")
